...
void memory_map_weights(TransformerWeights *w, const Config *p, float *ptr,
                        int shared_weights) {
...
int str_lookup(const char *str, const TokenIndex *sorted_vocab,
  ...
  const TokenIndex *res = (const TokenIndex *)bsearch(
  ...
  *n_tokens = 0;
  ...
  while (1) {
    ...
    for (int i = 0; i < (*n_tokens - 1); i++) {
      ...
      snprintf(str_buffer, (t->max_token_length * 2 + 1 + 2), "%s%s",
...
int sample_topp(const float *probabilities, int n, float topp,
    ...
    if (user_turn) {
      ...
      if (pos == 0 && system_prompt[0] != '\0') {
        const char system_template[] =
            "[INST] <<SYS>>\n%s\n<</SYS>>\n\n%s [/INST]";
        snprintf(rendered_prompt, 1152, system_template, system_prompt,
                 user_prompt);
      } else {

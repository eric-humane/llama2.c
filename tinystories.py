...
def setup_multi_gpu():
    ...
    if num_gpus > 1:
        print(f"Multiple GPUs detected ({num_gpus}), but not using distributed training.")
        print("For multi-GPU training, launch with: python -m torch.distributed.launch --nproc_per_node={num_gpus} tinystories.py ...")
...
def download(dataset_name: str, dataset_config: Optional[str] = None, 
            split: str = "train", text_field: str = "text", quiet: bool = False):
    """
    ...
    for i in tqdm(range(num_shards), disable=quiet, desc="Processing shards"):
        ...
        shard_end = (i + 1) * (len(dataset) // num_shards) if i < num_shards - 1 else len(dataset)
...
def process_shard(args, vocab_size, dataset_name, quiet=False):
    ...
    if not quiet:
        ...
        avg_seq_len = all_tokens.size / ((all_tokens == 1).sum() + 1e-10)  # Add small epsilon to avoid division by zero
...
def pretokenize(vocab_size, dataset_name: str, quiet=False):
    ...
    fun = partial(process_shard, vocab_size=vocab_size, dataset_name=dataset_name, quiet=quiet)
...
class PretokDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(self, split, max_seq_len, vocab_size, vocab_source, dataset_name: str, quiet=False):
        super().__init__()
        self.split = split
...
class Task:
    ...
    @staticmethod
    def iter_batches(batch_size, device=None, num_workers=None, quiet=False, dataset_name="TinyStories", **dataset_kwargs):
...
if __name__ == "__main__":
    ...
    parser.add_argument("stage", type=str, choices=["download", "pretokenize", "train_vocab", "fullclean"])
    parser.add_argument("--vocab_size", type=int, default=0, help="pretokenization vocab size. 0 = use Llama 2 tokenizer.")
    parser.add_argument("--dataset_name", type=str, default="roneneldan/TinyStories", help="Dataset name on Hugging Face")
    ...
    parser.add_argument("--text_field", type=str, default="text", help="Field containing the text in the dataset")
    parser.add_argument("--num_workers", type=int, default=None, help="Number of worker processes (default: auto)")
    parser.add_argument("--gpu_mem_fraction", type=float, default=None, help="Fraction of GPU memory to use (0.0-1.0)")
    ...
    if LOCAL_RANK in [-1, 0]:
        print(f"Device: {DEVICE}, CPU cores: {NUM_CPU_CORES}, Workers: {NUM_WORKERS_DEFAULT}")
    ...
    if args.stage == "download":
        download(dataset_name=args.dataset_name, dataset_config=args.dataset_config, 
